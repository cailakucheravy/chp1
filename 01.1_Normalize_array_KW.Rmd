...
sample_file_Jones     <- read.csv("Input/sample_list_whales_updated_Feb2024.csv")

# Sample list from array run in the Horvath lab: 
sample_file_Horvath   <- read.csv("Input/sample_list_whales_HORVATH_updated_Feb2024.csv") 
...
sample_sheet_Horvath2 <- sample_sheet_Horvath %>% 
  ...
  dplyr::rename(Order = OriginalOrderInBatch, sampleId = ExternalSampleID, block = Block, Species = SpeciesCommonName, plate = PlateNumber) %>% 
...
# ggsave("Plots/qc_plot_w_outliers.png", width = 6, height = 4.5, dpi = 300)
```
...
# ggsave("Plots/qc_plot_wo_outliers.png", width = 6, height = 4.5, dpi = 300)
...
We can also look for p-values to try and detect "bad" samples: 
```{r}
# Get raw betas 
Rawset <- minfi::preprocessRaw(RGset_outliersRemoved)
...
ggplot(data = QCdf, aes(x = mMed, y = uMed, label = rownames(df_qc_outliersRemoved))) + 
  geom_point() + 
  geom_text(size = 2, hjust = -0.1) +
  theme_bw() + 
  xlab("mMed") + ylab("uMed")
...
Call getBeta on Mset to return a data frame of normalized beta values for each CG:
```{r}
...
max(normalized_betas_minfi[,2:ncol(normalized_betas_minfi)])
min(normalized_betas_minfi[,2:ncol(normalized_betas_minfi)])
```

Save normalized betas: 
```{r}
#saveRDS(normalized_betas_minfi, 'output/nbetas_combined_KillerWhale_array_August2024.rds')
```
...
```{r}
#saveRDS(n_betas_t, 'output/tbetas_combined_KillerWhale_array_August2024.rds')

#n_betas_t <- readRDS("output/tbetas_combined_KillerWhale_array_August2024.rds")
#head(n_betas_t)
```
...
#sample_sheet_outliersRemoved <- readRDS("output/updated_sample_sheet_combined_KillerWhale_array.rds")
```
...
dim(n_betas_t)
pca_methylData <- prcomp(n_betas_t[,1:37492])
...
# Plot with colours for chip number, plate, location, sex, lab 
autoplot(pca_kw, 
         data = kw_pca_data, 
         colour = kw_pca_data$Year,
         label = TRUE, 
         shape = FALSE) 
```
...
```{r, warning = FALSE}
#normalized_betas_minfi <- readRDS("output/nbetas_combined_KillerWhale_array_August2024.rds")
...
We can see the technical variables of chip ID, Lab, and column may be influencing the data. Let's start by correcting for lab.
...
Combat batch correction for chip: 
```{r, warning = FALSE}
library(sva)
...
# Export as .rds
saveRDS(n_betas_corrected, 'output/tbetas_corrected_combined_KillerWhale_array_August2024.rds')
```
